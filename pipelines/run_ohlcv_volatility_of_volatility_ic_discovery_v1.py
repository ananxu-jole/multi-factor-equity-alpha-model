...
from pipelines.run_ohlcv_volatility_of_volatility_panel_generation_v1 import (
    PANEL_COLUMNS,
    TIMING_POLICY,
    validate_vov_panel_artifacts,
)


RUN_ID = "ohlcv_volatility_of_volatility_research_module_ic_discovery_v1"
SOURCE_PANEL_ROOT = Path("artifacts/research/ohlcv_volatility_of_volatility_research_module_v1/panel_v1")
OUT_DIR = Path("artifacts/research/ohlcv_volatility_of_volatility_research_module_v1/ic_discovery_v1")
...
RESEARCH_ONLY_GUARDRAIL = (
    "Phase 8 research-only IC discovery for audited VoV panels. No formula changes, "
    "panel regeneration, Family B/C work, refinement, validation, governance mutation, "
    "production registration, threshold change, or ML work is performed."
)
...
def _load_panel_manifest(panel_root: Path = SOURCE_PANEL_ROOT) -> pd.DataFrame:
    ok, errors = validate_vov_panel_artifacts(panel_root)
    if not ok:
        raise ValueError("approved VoV panel validation failed: " + "; ".join(errors))
    ...
    if ids != IMPLEMENTED_CANDIDATE_IDS:
        raise ValueError(f"VoV panel manifest candidate IDs mismatch: {ids}")
    if any(str(candidate_id).startswith(BLOCKED_FAMILY_PREFIXES) for candidate_id in ids):
        raise ValueError("Family B/C candidate appeared in VoV panel manifest")
    required = {
        "candidate_id",
        "source_spec_id",
        "module_id",
    ...
    if missing:
        raise ValueError("VoV panel manifest missing required columns: " + ", ".join(sorted(missing)))
    if set(manifest["module_id"].astype(str)) != {MODULE_ID}:
        raise ValueError("VoV panel manifest module_id mismatch")
    if set(manifest["timing_policy"].astype(str)) != {TIMING_POLICY}:
        raise ValueError("VoV panel manifest timing policy mismatch")
    if int(manifest["duplicate_key_count"].sum()) != 0:
        raise ValueError("VoV panel manifest reports duplicate keys")
    return manifest
...
def _load_candidate_panel(path: str | Path, candidate_id: str) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    if set(panel_long["candidate_id"].astype(str)) != {candidate_id}:
        raise ValueError(f"candidate panel {path} candidate_id mismatch")
    if any(str(value).startswith(BLOCKED_FAMILY_PREFIXES) for value in panel_long["candidate_id"].dropna().unique()):
        raise ValueError(f"Family B/C candidate appeared in {path}")
    if set(panel_long["module_id"].astype(str)) != {MODULE_ID}:
        raise ValueError(f"candidate panel {path} module_id mismatch")
    if set(panel_long["family"].astype(str)) != {FAMILY}:
        raise ValueError(f"candidate panel {path} family mismatch")
    if set(panel_long["timing_policy"].astype(str)) != {TIMING_POLICY}:
        raise ValueError(f"candidate panel {path} timing policy mismatch")
...
def rolling_ic_diagnostics(daily_ic: pd.DataFrame) -> pd.DataFrame:
    ...
    for (candidate_id, horizon), group in daily_ic.sort_values("date").groupby(["candidate_id", "horizon"]):
        out = group[["date", "candidate_id", "source_spec_id", "family", "horizon"]].copy()
        series = group["ic"]
...
def classify_candidate(mean_ic: float, ic_ir: float, positive_ic_rate: float, coverage_ratio: float) -> str:
    if (
        pd.notna(mean_ic)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Phase 8 VoV research-only IC discovery.")
    ...
    print(f"Wrote VoV Phase 8 IC discovery artifacts for {len(IMPLEMENTED_CANDIDATE_IDS)} candidates to {args.out_dir}")
    print(
        rankings[
            [
                "rank",
