"""Run extracted 04A -> 04B -> 07 -> 08 -> 09 -> 09B engines in order."""
...
STAGES = [
    ("04a_alpha_construction", run_04a_alpha_construction),
...
def _count_from_result(result: dict[str, object], artifact_key: str, summary_metric: str) -> object:
    artifact = result.get(artifact_key)
    if artifact is not None:
        return len(artifact)
    value = _metric_value(result.get("summary"), summary_metric)
    return "" if value is None else value


def _approved_04a_count(result: dict[str, object]) -> object:
    value = _metric_value(result.get("summary"), "approved_for_alpha_validation_count")
    if value is not None:
        return value
    quality = result.get("alpha_construction_quality")
    if quality is not None and "status" in quality.columns:
        return int(quality["status"].eq("APPROVED_FOR_ALPHA_VALIDATION").sum())
    return ""
...
def _print_stage_summary(stage_id: str, elapsed_seconds: float, result: dict[str, object]) -> None:
    ...
    print(f"  approved_04a_count: {_approved_04a_count(result)}")
    print(f"  wfv_gate_rows: {_count_from_result(result, 'alpha_wfv_gate', 'wfv_gate_rows')}")
    print(f"  stress_gate_rows: {_count_from_result(result, 'stress_gate', 'stress_gate_rows')}")
...
def _print_description() -> None:
    print("combined_pipeline: alpha_to_portfolio_full")
    print("stages:")
...
def verify_alpha_to_portfolio_full(results: dict[str, dict[str, object]]) -> list[dict[str, object]]:
    stage_04a = results["04a_alpha_construction"]
    ...
    alpha_quality = stage_04a["alpha_construction_quality"]
    ...
    approved_04a_count = int(alpha_quality["status"].eq("APPROVED_FOR_ALPHA_VALIDATION").sum())
    approved_or_watch_count = int(wfv_gate["status"].isin(valid_wfv_statuses).sum())
    final_core_names = set(final_core["alpha_name"].dropna())
    portfolio_alpha_names = set(portfolio_alpha_pool["alpha_name"].dropna())

    return [
        {
            "check_name": "04a_has_approved_for_alpha_validation",
            "passed": approved_04a_count > 0,
            "details": f"approved_04a_count={approved_04a_count}",
        },
        {
...
def run_alpha_to_portfolio_full(
    *,
    db_path=None,
    write: bool,
    verbose: bool,
    ...
    if not all(check["passed"] for check in checks):
        raise ValueError("alpha_to_portfolio_full verification failed")
    return results


def main() -> int:
    parser = argparse.ArgumentParser(description="Run extracted 04A -> 04B -> 07 -> 08 -> 09 -> 09B engines.")
