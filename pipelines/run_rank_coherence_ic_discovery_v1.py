...
RUN_ID = "rank_coherence_ic_discovery_v1"
...
RESEARCH_ONLY_GUARDRAIL = (
    "Research-only IC discovery pass for the approved six-candidate rank-coherence subset. "
    "No refinement, validation, governance mutation, threshold change, production registration, "
    "ML, promotion, or demotion is performed."
)
...
def _load_manifest_subset() -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Approved rank-coherence candidates missing from panel manifest: {missing}")
    unexpected = sorted(set(subset["candidate_id"]) - set(APPROVED_CANDIDATES))
    if unexpected:
        raise ValueError(f"Unexpected candidates in approved subset: {unexpected}")
    if len(subset) != len(APPROVED_CANDIDATES):
        raise ValueError("Approved rank-coherence subset did not resolve to exactly 6 manifest rows.")
...
def _load_candidate_panel(path: str) -> pd.DataFrame:
    ...
    required = {"date", "ticker", "signal_value", "candidate_id", "family", "theme", "horizon"}
...
def _load_candidate_panels(subset: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {rec["signal_name"]: _load_candidate_panel(rec["panel_path"]) for rec in subset.to_dict(orient="records")}
...
def _coverage_diagnostics(panels: dict[str, pd.DataFrame], close: pd.DataFrame, subset: pd.DataFrame) -> pd.DataFrame:
    metadata = subset.set_index("signal_name")[["candidate_id", "family", "theme", "horizon"]].to_dict("index")
        ...
        rows.append(
            {
                ...
                "theme": rec["theme"],
                "declared_horizon": rec["horizon"],
                "panel_dates": int(len(panel.index)),
                "panel_tickers": int(len(panel.columns)),
                "non_null_observations": int(panel.notna().sum().sum()),
                "close_aligned_dates": int(len(aligned.index)),
...
def _candidate_summary(scores: pd.DataFrame, redundancy: pd.DataFrame, coverage: pd.DataFrame) -> pd.DataFrame:
    wide = scores.pivot_table(
        index=["candidate_id", "signal_name", "family", "theme", "declared_horizon"],
        columns="scored_horizon",
        values="mean_ic",
    )
...
def _theme_summary(scores: pd.DataFrame) -> pd.DataFrame:
    return (
        scores.groupby(["theme", "scored_horizon"], as_index=False)
        .agg(
            candidate_count=("candidate_id", "nunique"),
            mean_ic=("mean_ic", "mean"),
            median_ic=("mean_ic", "median"),
        ...
        .sort_values(["theme", "scored_horizon"])
    )
...
def main() -> int:
    ...
    coverage = _coverage_diagnostics(panels, close, subset)
    ...
    theme_summary = _theme_summary(scores)
    ...
    theme_summary.to_csv(OUT_DIR / "family_theme_summary.csv", index=False)
        ...
        json.dumps(
            {
                ...
                "held_back_candidate_ids": HELD_BACK_CANDIDATES,
                ...
                "outputs": {
                    ...
                    "family_theme_summary": str(OUT_DIR / "family_theme_summary.csv"),
    ...
    print(f"Wrote research-only rank-coherence IC discovery outputs for {len(APPROVED_CANDIDATES)} candidates to {OUT_DIR}")
