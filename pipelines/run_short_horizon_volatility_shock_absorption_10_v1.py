...
RUN_ID = "short_horizon_volatility_shock_absorption_10_v1"
OUT_DIR = Path("artifacts/research") / RUN_ID
NOTE_PATH = Path("docs/research_notes/short_horizon_volatility_shock_absorption_10_v1.md")
...
IDIOSYNCRATIC_NOTE = Path("docs/research_notes/idiosyncratic_stress_containment_10_v1.md")
...
SIGNAL_NAME = "short_horizon_volatility_shock_absorption_10"
...
def _state_flags(
    ...
    recent_vol_shock = vol_shock.rolling(10, min_periods=1).max().astype(bool)
    ...
    states["VOLATILITY_SHOCK_ABSORBING"] = (
        market_vol_absorbing & contained_dispersion & not_breadth_repair_cluster
    ).fillna(False)
    ...
    states["VOLATILITY_SHOCK_OUTSIDE_WEAK_BREADTH"] = (
        market_vol_absorbing & ~stress["weak_breadth"].fillna(False)
    ).fillna(False)
...
def build_volatility_shock_signal(
    panels: dict[str, pd.DataFrame],
    benchmark: pd.Series,
    ...
    shock_present = (
        _safe_div(range20, range60).rank(axis=1, pct=True)
        * _safe_div(residual_vol20, residual_vol60).rank(axis=1, pct=True)
    ).clip(lower=0.0)
    fast_absorption = (
        (1.0 - _safe_div(range5, range20).rank(axis=1, pct=True))
        * (1.0 - _safe_div(residual_vol5, residual_vol20).rank(axis=1, pct=True))
    ).clip(lower=0.0)
    no_panic_extension = (
        (1.0 - ret5.rank(axis=1, pct=True).sub(0.5).abs() * 2.0)
        * (1.0 - ret10.rank(axis=1, pct=True).sub(0.5).abs() * 2.0)
    ).clip(lower=0.0)
    residual_shock_absorption = (
        (1.0 - residual5.rank(axis=1, pct=True).sub(0.5).abs() * 2.0)
        * (1.0 - residual10.rank(axis=1, pct=True).sub(0.5).abs() * 2.0)
    ).clip(lower=0.0)
    ...
    states = _state_flags(panels, benchmark, absorption_quality)
    vol_gate_raw = (
        (shock_present > 0.55)
        & (fast_absorption > fast_absorption.stack().quantile(0.50))
        & (range_containment > range_containment.stack().quantile(0.45))
    ).astype(float)
    state_gate = _market_state_panel(states["VOLATILITY_SHOCK_ABSORBING"], close.columns)
    gate = vol_gate_raw * state_gate

    signal = _rank_cs(absorption_quality * gate)
    for exposure in [
        _rank_cs(ret5),
        _rank_cs(ret10),
        _rank_cs(ret15),
    ...
    diagnostics = pd.DataFrame(
        {
            "component": [
                ...
                "close_support",
                "liquidity_sufficient",
                "volatility_shock_gate",
                "absorption_quality",
                "final_signal",
            ],
            "finite_pct": [
                float(shock_present.notna().mean().mean()),
                float(fast_absorption.notna().mean().mean()),
                float(no_panic_extension.notna().mean().mean()),
                float(residual_shock_absorption.notna().mean().mean()),
                ...
                float(absorption_quality.notna().mean().mean()),
                float(signal.notna().mean().mean()),
            ],
            "mean_abs": [
                float(shock_present.abs().mean().mean()),
                float(fast_absorption.abs().mean().mean()),
                float(no_panic_extension.abs().mean().mean()),
                float(residual_shock_absorption.abs().mean().mean()),
                ...
                float(absorption_quality.abs().mean().mean()),
...
def reference_panels(signals: dict[str, pd.DataFrame], panels: dict[str, pd.DataFrame], benchmark: pd.Series) -> dict[str, pd.DataFrame]:
    ...
    short_vol_shock_absorption_proxy = _rank_cs(
        (
            _safe_div(true_range.rolling(20, min_periods=12).mean(), true_range.rolling(60, min_periods=40).mean()).rank(axis=1, pct=True)
            * (
                1.0
    ...
    refs["short_vol_shock_absorption_proxy"] = short_vol_shock_absorption_proxy.reindex(index=first.index, columns=first.columns)
...
def volatility_shock_corr_summary(orth: pd.DataFrame) -> pd.DataFrame:
    ...
    for name, group in orth.groupby("signal_name"):
        ...
        comparisons = [
            ...
            "short_vol_shock_absorption_proxy",
        ...
        idio_refs = group[group["comparison"].isin(["idiosyncratic_stress_proxy"])]
        vol_absorption_refs = group[group["comparison"].isin(["short_vol_shock_absorption_proxy"])]
        ...
        row["max_idiosyncratic_stress_corr"] = float(idio_refs["abs_value_corr"].max()) if not idio_refs.empty else np.nan
        row["max_vol_shock_absorption_corr"] = float(vol_absorption_refs["abs_value_corr"].max()) if not vol_absorption_refs.empty else np.nan
...
def classify_candidate(
    ...
    vol_counts = (
        state_attr[
            state_attr["state"].isin(
                [
                    "VOLATILITY_SHOCK_ACTIVE",
                    "RECENT_VOLATILITY_SHOCK",
                    "VOLATILITY_SHOCK_ABSORBING",
                    "VOLATILITY_SHOCK_ABSORPTION_WITH_QUALITY",
                    "VOLATILITY_SHOCK_OUTSIDE_WEAK_BREADTH",
                    "CONTAINED_DISPERSION_VOL_SHOCK",
                    "MARKET_VOL_ABSORBING",
                ]
            )
        ]
        .groupby("signal_name")["mean_ic"]
        .agg(positive_volatility_shock_state_count=lambda s: int((s > 0.004).sum()), best_volatility_shock_state_ic="max")
        .reset_index()
    )
    hostile_counts = (
        state_attr[
    ...
    summary = (
        ...
        .merge(vol_counts, on="signal_name", how="left")
    ...
    for _, row in summary.iterrows():
        ...
        if h10_mean < 0.008:
            ...
            issues.append("h10_below_validation_ready_floor")
        if h10_pos < 0.525:
            issues.append("weak_h10_positive_ic_rate")
        if int(row["horizon"]) == 5 and row.get("h5_mean_ic", np.nan) > row.get("h10_mean_ic", np.nan):
        ...
        if row.get("max_idiosyncratic_stress_corr", 0) > 0.35:
            issues.append("idiosyncratic_stress_similarity_risk")
        ...
        if row.get("positive_volatility_shock_state_count", 0) < 2:
            issues.append("weak_volatility_shock_state_support")
        ...
        if (
            ...
            and row.get("positive_volatility_shock_state_count", 0) >= 3
        ):
            status = "CANDIDATE_FOR_CONDITIONAL_VALIDATION"
        elif (
            h10_mean > 0.008
            and h10_pos >= 0.55
            and row.get("positive_volatility_shock_state_count", 0) >= 2
            ...
            row.get("positive_volatility_shock_state_count", 0) >= 2
        ...
        rows.append(
            {
                ...
                "family": "short_horizon_volatility_shock_absorption",
                ...
                "max_idiosyncratic_stress_corr": row.get("max_idiosyncratic_stress_corr"),
                ...
                "short_vol_shock_absorption_proxy_corr": row.get("short_vol_shock_absorption_proxy_corr"),
                ...
                "positive_volatility_shock_state_count": int(row.get("positive_volatility_shock_state_count", 0) or 0),
                "best_volatility_shock_state_ic": row.get("best_volatility_shock_state_ic"),
...
def _decision_text(decision: pd.Series) -> str:
    ...
    if status == "CANDIDATE_FOR_CONDITIONAL_VALIDATION":
        return "`short_horizon_volatility_shock_absorption_10` should move to formal conditional validation using this fixed single formulation."
    if status == "CONDITIONAL_REFINEMENT_CANDIDATE":
        return "`short_horizon_volatility_shock_absorption_10` should receive a narrow refinement diagnostics pass focused on h10 strength, volatility-inventory separation, and activation coverage."
    if status == "CONDITIONAL_ONLY_RESEARCH":
        return "`short_horizon_volatility_shock_absorption_10` should remain conditional-only research evidence until h10 strength or volatility-shock state support improves."
    return "`short_horizon_volatility_shock_absorption_10` should be rejected in this formulation before more Expansion v5 implementation."
...
def write_note(
    ...
    lines = [
        ...
        f"- Idiosyncratic stress containment v1: `{IDIOSYNCRATIC_NOTE}`",
        ...
        "",
        f"- Genuinely short-horizon volatility shock absorption: positive volatility-shock state count was `{int(decision['positive_volatility_shock_state_count'])}` and best volatility-shock state IC was `{_fmt(decision['best_volatility_shock_state_ic'])}`.",
        f"- Broad drawdown-pressure risk: max drawdown-pressure correlation was `{_fmt(decision['max_drawdown_pressure_corr'])}`.",
        f"- Idiosyncratic stress similarity risk: max idiosyncratic-stress correlation was `{_fmt(decision['max_idiosyncratic_stress_corr'])}`.",
...
def main() -> None:
    ...
    signals, states, component_diagnostics = build_volatility_shock_signal(panels, benchmark)
    registry = pd.DataFrame(
        [
            {
                ...
                "mechanism_thesis": "Shorter-horizon repair from absorption of recent volatility and range shocks.",
                "volatility_shock_definition": "Recent benchmark volatility spike or market range expansion with individual range and residual volatility stress.",
                "absorption_stabilization_logic": "Fast range contraction, residual vol contraction, rank stabilization, close support, and sufficient liquidity.",
                "differs_from_inventory": "Short-horizon shock absorption rather than h20 post-stress volatility compression.",
                "differs_from_reversal_momentum": "Avoids extreme winners/losers and neutralizes reversal, momentum, and residual momentum exposures.",
                "expected_activation_state": "VOLATILITY_SHOCK_ABSORBING",
        ...
        json.dumps(
            {
                ...
                "idiosyncratic_note": str(IDIOSYNCRATIC_NOTE),
