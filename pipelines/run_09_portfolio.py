"""CLI for the 09 portfolio construction engine."""
...
from src.portfolio.construction import (  # noqa: E402
    ...
    TOP_QUANTILE,
    run_09_portfolio_construction,
)
from src.portfolio_storage import DYNAMIC_PORTFOLIO_TABLES  # noqa: E402


def _print_stage_description() -> None:
    stage = get_stage("09_portfolio_construction")
    ...
    print("engine_parameters:")
    print(f"  max_abs_weight: {MAX_ABS_WEIGHT}")
    print(f"  top_quantile: {TOP_QUANTILE}")
    print(f"  bottom_quantile: {BOTTOM_QUANTILE}")
    print(f"  gross_exposure: {GROSS_EXPOSURE}")
    print(f"  rebalance_frequency: {REBALANCE_FREQUENCY}")
    print(f"  cost_bps: {COST_BPS}")
    print(f"  execution_lag: {EXECUTION_LAG}")
    print(f"  portfolio_modes: {', '.join(PORTFOLIO_MODES)}")
    print("expected_input_tables:")
    for table in REQUIRED_INPUT_TABLES:
        print(f"  - {table}")
    print("expected_output_tables:")
    for artifact, (current_table, history_table) in DYNAMIC_PORTFOLIO_TABLES.items():
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run or describe the 09 portfolio construction stage.")
    ...
    parser.add_argument(
        ...
        help="Portfolio version label to stamp on outputs.",
    )
    ...
    result = run_09_portfolio_construction(
