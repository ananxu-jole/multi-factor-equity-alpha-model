...
RUN_ID = "drawdown_pressure_stabilization_10_v1"
OUT_DIR = Path("artifacts/research") / RUN_ID
NOTE_PATH = Path("docs/research_notes/drawdown_pressure_stabilization_10_v1.md")
...
SIGNAL_NAME = "drawdown_pressure_stabilization_10"
...
def _state_flags(
    ...
    bench_peak120 = benchmark.rolling(120, min_periods=60).max()
    bench_drawdown120 = benchmark / bench_peak120 - 1.0
    bench_downside10 = bench_ret1.clip(upper=0.0).rolling(10, min_periods=7).sum()
    bench_downside30 = bench_ret1.clip(upper=0.0).rolling(30, min_periods=20).sum()
    downside_pressure_reducing = (bench_downside10 > bench_downside30 / 3.0).fillna(False)

    breadth20 = (ret20 > 0).mean(axis=1)
    dispersion20 = ret20.std(axis=1)
    drawdown_pressure = (
        stress["drawdown_acceleration"].fillna(False)
        | (bench_drawdown120 < _rolling_quantile(bench_drawdown120, 0.35))
        | ((benchmark < bench_ma60) & (bench_ret20 < -0.02))
    ).fillna(False)
    active_drawdown_pressure = (drawdown_pressure | (stress_recent_20 & (bench_ret10 < 0))).fillna(False)
    not_breadth_repair_cluster = (~stress["weak_breadth"].fillna(False) | (breadth20.diff(10) <= 0.04)).fillna(False)
    ...
    states["ACTIVE_DRAWDOWN_PRESSURE"] = active_drawdown_pressure
    states["DRAWDOWN_PRESSURE_STABILIZING"] = (
        active_drawdown_pressure & downside_pressure_reducing & contained_dispersion
    ).fillna(False)
    states["DRAWDOWN_STABILIZATION_WITH_QUALITY"] = (
        active_drawdown_pressure & downside_pressure_reducing & contained_dispersion & high_quality_dates
    ).fillna(False)
    states["DRAWDOWN_PRESSURE_NOT_WEAK_BREADTH_REPAIR"] = (
        active_drawdown_pressure & not_breadth_repair_cluster
    ).fillna(False)
    states["DOWNSIDE_PRESSURE_REDUCING"] = (active_drawdown_pressure & downside_pressure_reducing).fillna(False)
    states["CONTAINED_DISPERSION_DRAWDOWN"] = (active_drawdown_pressure & contained_dispersion).fillna(False)
    states["ACTIVE_HOSTILE_OR_STRESS"] = (
        stress_recent_20 | stress["weak_breadth"].fillna(False) | ((benchmark < bench_ma60) & (bench_ret20 < 0))
    ).fillna(False)
...
def build_drawdown_pressure_signal(
    panels: dict[str, pd.DataFrame],
    benchmark: pd.Series,
    ...
    downside_pressure_reduction = (1.0 - _safe_div(downside10.abs(), (downside30.abs() / 3.0)).rank(axis=1, pct=True)).clip(lower=0.0)

    residual_rank20 = residual20.rank(axis=1, pct=True)
    pressure_present_not_extreme = (1.0 - residual_rank20.sub(0.35).abs() * 3.0).clip(lower=0.0)
    not_short_chase = (1.0 - residual5.rank(axis=1, pct=True).sub(0.5).abs() * 2.0).clip(lower=0.0)
    ...
    stabilization_quality = (
        downside_pressure_reduction
        * pressure_present_not_extreme
        * not_short_chase
        * range_containment
        * residual_vol_normalization
        * close_support
    ...
    gate = _market_state_panel(states["DRAWDOWN_PRESSURE_STABILIZING"], close.columns)
    ...
    diagnostics = pd.DataFrame(
        {
            "component": [
                ...
                "close_support",
                "liquidity_sufficient",
                "drawdown_pressure_gate",
                "stabilization_quality",
                "final_signal",
            ],
            "finite_pct": [
                float(downside_pressure_reduction.notna().mean().mean()),
                float(pressure_present_not_extreme.notna().mean().mean()),
                float(not_short_chase.notna().mean().mean()),
            ...
            "mean_abs": [
                float(downside_pressure_reduction.abs().mean().mean()),
                float(pressure_present_not_extreme.abs().mean().mean()),
                float(not_short_chase.abs().mean().mean()),
...
def drawdown_corr_summary(orth: pd.DataFrame) -> pd.DataFrame:
...
def classify_candidate(
    ...
    drawdown_counts = (
        state_attr[
            state_attr["state"].isin(
                [
                    "ACTIVE_DRAWDOWN_PRESSURE",
                    "DRAWDOWN_PRESSURE_STABILIZING",
                    "DRAWDOWN_STABILIZATION_WITH_QUALITY",
                    "DRAWDOWN_PRESSURE_NOT_WEAK_BREADTH_REPAIR",
                    "DOWNSIDE_PRESSURE_REDUCING",
                    "CONTAINED_DISPERSION_DRAWDOWN",
                ]
            )
        ]
        .groupby("signal_name")["mean_ic"]
        .agg(positive_drawdown_state_count=lambda s: int((s > 0.004).sum()), best_drawdown_state_ic="max")
        .reset_index()
    )
    hostile_counts = (
        state_attr[
    ...
    summary = (
        ...
        .merge(drawdown_counts, on="signal_name", how="left")
    ...
    for _, row in summary.iterrows():
        ...
        if row.get("positive_drawdown_state_count", 0) < 2:
            issues.append("weak_drawdown_state_support")
        ...
        if (
            ...
            and row.get("positive_drawdown_state_count", 0) >= 3
        ):
            status = "CANDIDATE_FOR_CONDITIONAL_VALIDATION"
        elif (
            h10_mean > 0.009
            and h10_pos >= 0.525
            and row.get("positive_drawdown_state_count", 0) >= 2
            ...
            row.get("positive_drawdown_state_count", 0) >= 2
        ...
        rows.append(
            {
                ...
                "family": "drawdown_pressure_stabilization",
                ...
                "positive_drawdown_state_count": int(row.get("positive_drawdown_state_count", 0) or 0),
                "best_drawdown_state_ic": row.get("best_drawdown_state_ic"),
...
def _decision_text(decision: pd.Series) -> str:
    ...
    if status == "CANDIDATE_FOR_CONDITIONAL_VALIDATION":
        return "`drawdown_pressure_stabilization_10` should move to formal conditional validation using this fixed single formulation."
    if status == "CONDITIONAL_REFINEMENT_CANDIDATE":
        return "`drawdown_pressure_stabilization_10` should receive a narrow refinement diagnostics pass focused on h10 strength, activation breadth, and reversal separation."
    if status == "CONDITIONAL_ONLY_RESEARCH":
        return "`drawdown_pressure_stabilization_10` should remain conditional-only research evidence until h10 strength or drawdown-state support improves."
    return "`drawdown_pressure_stabilization_10` should be rejected in this formulation before moving to the next Expansion v5 concept."
...
def write_note(
    ...
    lines = [
        ...
        "",
        f"- Genuinely drawdown-pressure stabilization: positive drawdown-state count was `{int(decision['positive_drawdown_state_count'])}` and best drawdown-state IC was `{_fmt(decision['best_drawdown_state_ic'])}`.",
...
def main() -> None:
    ...
    signals, states, component_diagnostics = build_drawdown_pressure_signal(panels, benchmark)
    registry = pd.DataFrame(
        [
            {
                ...
                "mechanism_thesis": "Shorter-horizon repair from stabilizing downside pressure during drawdown-pressure regimes.",
                "drawdown_pressure_definition": "Benchmark drawdown acceleration or negative benchmark drawdown/trend pressure.",
                "stabilization_confirmation_logic": "Residual downside pressure reduction, contained range, residual vol normalization, close support, and sufficient liquidity.",
                "differs_from_inventory": "Drawdown-pressure gate rather than participation, breadth, liquidity repair, or h20 volatility compression.",
                "differs_from_reversal_momentum": "Avoids extreme damage and neutralizes reversal, momentum, and residual momentum exposures.",
                "expected_activation_state": "DRAWDOWN_PRESSURE_STABILIZING",
    ...
    orth_summary = drawdown_corr_summary(orth)
