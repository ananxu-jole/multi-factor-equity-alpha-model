...
import pipelines.run_ohlcv_volatility_of_volatility_refinement_ic_discovery_v1 as runner
...
def _write_panel_root(tmp_path: Path) -> tuple[Path, Path]:
    ...
    close = pd.DataFrame(
        {
            ticker: 100 + idx * 0.25 + np.arange(len(dates)) * (0.06 + idx / 1800)
    ...
    for cidx, candidate_id in enumerate(IMPLEMENTED_REFINEMENT_IDS):
        ...
        for didx, date in enumerate(dates):
            for tidx, ticker in enumerate(tickers):
                warmup = didx < 5
                active = not warmup and ((tidx + cidx) % 4 != 0)
                base_signal = (tidx + 1) / len(tickers)
                if candidate_id in {"vov_01_ref_longer_memory", "vov_03_ref_extension_controlled"}:
                    signal = base_signal
                elif candidate_id.endswith("strict_calm") or candidate_id.endswith("strict_chop"):
                    signal = 1.0 - base_signal
                else:
                    signal = base_signal * 0.8 + 0.1
...
def test_run_refinement_ic_discovery_writes_required_artifacts(tmp_path):
    ...
    assert set(rankings["recommendation"]).issubset(
        {"ADVANCE_TO_VALIDATION_DESIGN", "WATCH", "REJECT"}
    )
    assert "best_primary_mean_ic_delta_vs_anchor" in rankings.columns
...
def test_candidate_summary_is_candidate_level_not_horizon_grain(tmp_path):
    ...
    assert len(summary) == len(IMPLEMENTED_REFINEMENT_IDS)
    assert len(written) == len(IMPLEMENTED_REFINEMENT_IDS)
    assert "horizon" not in summary.columns
    assert "primary_mean_ic" in summary.columns
    assert "primary_best_delta_vs_anchor" in summary.columns
...
def test_manifest_guardrails_are_fail_closed(tmp_path):
    ...
    assert manifest["blocked_candidates_used"] is False
...
def test_blocked_candidate_in_manifest_is_rejected(tmp_path):
    panel_root, close_path = _write_panel_root(tmp_path)
    manifest = pd.read_csv(panel_root / "panel_manifest.csv")
    manifest.loc[0, "candidate_id"] = "vov_05"
    ...
    try:
        ...
        raise AssertionError("blocked candidate should have failed IC discovery preflight")
...
def test_candidate_ranking_compares_against_family_anchors(tmp_path):
    panel_root, close_path = _write_panel_root(tmp_path)
    out_dir = tmp_path / "ic_discovery_v1"
    runner.run_ic_discovery(panel_root=panel_root, close_path=close_path, out_dir=out_dir)
    rankings = pd.read_csv(out_dir / "candidate_rankings.csv")
    ...
    assert list(rankings["rank"]) == list(range(1, len(IMPLEMENTED_REFINEMENT_IDS) + 1))
    assert set(rankings["anchor_candidate_id"]) == {"vov_01_ref_anchor", "vov_03_ref_anchor"}
